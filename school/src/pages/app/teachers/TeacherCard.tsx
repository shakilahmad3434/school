...
interface TeacherCardInterface {
    ...
    subjects: string[];
    id: string;
    setUpdateTeacherValue?: (data: string) => void;
    setDeleteTeacherValue?: (data: string) => void;
}

const TeacherCard: FC<TeacherCardInterface> = ({
    dob,
    teacherPhoto,
    teacherName,
    qualification,
            ...
            <div className="pt-10 pb-6 px-6">
                <div className="text-center mb-6">
                    ...
                    <div className="flex items-center justify-center space-x-2 text-gray-600 text-sm mb-4">
                        <span className="bg-gradient-to-r from-yellow-500 to-yellow-600 text-white px-4 py-2 rounded-full text-xs font-medium hover:from-pink-600 hover:to-pink-700 transition-all duration-300 cursor-pointer transform hover:scale-105 capitalize">
                            {qualification}
                        </span>
                    </div>
                ...
                <div className="mb-6">
                    ...
                    <div className="flex flex-wrap gap-2">
                        {subjects?.map((subject: string, subIndex: number) => (
                            <span
                                key={subIndex}
                                className={`bg-gradient-to-r from-${
                                    colors[subIndex % colors.length]
                                }-500 to-${
                                    colors[subIndex % colors.length]
                                }-600 text-white px-4 py-2 capitalize rounded-full text-xs font-medium hover:from-${
                                    colors[subIndex % colors.length]
                                }-600 hover:to-${
                                    colors[subIndex % colors.length]
                                }-700 transition-all duration-300 cursor-pointer transform hover:scale-105`}
                            >
                                {subject}
                            </span>
                        ))}
                ...
                <div className="flex space-x-3">
                    <button
                        onClick={() => setUpdateTeacherValue?.(id)}
                        className="flex-1 bg-gray-100 hover:bg-gray-200 text-gray-800 font-medium py-2.5 px-4 rounded-lg transition-all duration-200 flex items-center justify-center space-x-2 border border-gray-200"
                    >
                        <i className="ri-edit-2-line text-sm"></i>
                        <span>Edit</span>
                    </button>
                    <button
                        onClick={() => setDeleteTeacherValue?.(id)}
